    ...
    void CalPPRFwd(IndexType svid, ValueType alpha = ALPHA)
    {
        ...
        while (1)
        {
            ...
            for (IndexType u = 0; u < vertex_count; ++u)
            {
                ...
                if (u == svid)
                    pr[oid][u] += alpha * 1.0;
                if (std::fabs(pr[oid][u] - pr[id][u]) > 1e-14)
                    stop = false;
            }
            if (stop)
                break;
        ...
        while (1)
        {
            ...
            for (IndexType u = 0; u < vertex_count; ++u)
            {
                ...
                for (size_t j = 0; j < col_ind[u].size(); ++j)
                {
                    ...
                    IndexType current_out_degree_u = col_ind[u].size();
                    IndexType denominator = (current_out_degree_u == 0) ? 1 : current_out_degree_u;
                    pr[oid][u] += pr[id][v] / denominator;
                }

                pr[oid][u] = (1.0 - alpha) * pr[oid][u];
                if (u == source_vertex_id)
                    pr[oid][u] += alpha * 1.0;
                if (std::fabs(pr[oid][u] - pr[id][u]) > 1e-14)
                    stop = false;
            }
